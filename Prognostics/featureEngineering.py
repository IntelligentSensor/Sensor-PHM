...
class featureEngineer(object):
    ...
    x_train, y_train, x_valid, y_valid, x_test, y_test = test.prepro(d_path=path,
    ...
    #plan2 :SMOTE   默认生成1:1的数据
    #对于少数类样本a, 随机选择一个最近邻的样本b, 然后
    #从a与b的连线上随机选取一个点c作为新的少数类样本;
    ...
    #plan3 :smote的变体：Borderline
    #样本的近邻至少有一半是其他类，（此时样本被称为危险样本）
    #最近邻中的随机样本b与该少数类样本a来自于不同的类
    def Smote_bd(data,label):
    ...
    sts.shapiro(sample)  #Shapiro-Wilk 正态显著性检验
    sts.anderson(sample,dist="logistic")  #Anderson-Darling
    ...
    def pywtEigenvector(oneSample):    #小波包变换
    ...
    def generatePywt(data):
        length = len(data)
        eigenvectorSet = []
        for i in range(length):
            eigenvectorSet.append(statsEigenvector(data[i]))
        
        return np.array(eigenvectorSet)

    def mywavelet(oneSample):             #手写小波包变换
    ...
    for i in range(y_train.shape[1]):
        head.append(np.argwhere(y_train_decode == i)[0][0])

    time_index = np.array([i for i in range(2048)])               #时间index

    form_1 = np.vstack((time_index,  x_train[head[0]]))        #溶液地未连接
    form_2 = np.vstack((time_index,  x_train[head[1]]))        #流通池接地
    form_3 = np.vstack((time_index,  x_train[head[2]]))        #电缆线未连接
    form_4 = np.vstack((time_index,  x_train[head[3]]))        #球泡破裂
    form_5 = np.vstack((time_index,  x_train[head[4]]))        #支架损坏
    form_6 = np.vstack((time_index,  x_train[head[5]]))        #电极污染
    form_7 = np.vstack((time_index,  x_train[head[6]]))        #电解液缺失
    form_8 = np.vstack((time_index,  x_train[head[7]]))        #水样波动
    normal = np.vstack((time_index,  x_train[head[8]]))        #正常
    ...
    def generateFrechet(data):
        Frechet_1 = Frechet(data, form_1)
        Frechet_2 = Frechet(data, form_2)
        Frechet_3 = Frechet(data, form_3)
        Frechet_4 = Frechet(data, form_4)
        Frechet_5 = Frechet(data, form_5)
        Frechet_6 = Frechet(data, form_6)
        Frechet_7 = Frechet(data, form_7)
        Frechet_8 = Frechet(data, form_8)
        
        return np.hstack((Frechet_1, Frechet_2, Frechet_3, Frechet_4, Frechet_5, Frechet_6, Frechet_7, Frechet_8))

    '生成特征'
    feature_train = np.hstack((generateStats(x_train), generateWave_L2(x_train), generateFrechet(x_train)))
    feature_valid = np.hstack((generateStats(x_valid), generateWave_L2(x_valid), generateFrechet(x_valid)))
    feature_test = np.hstack((generateStats(x_test), generateWave_L2(x_test), generateFrechet(x_test)))
    feature_train.shape
    ...
    def var_filter(data, label, k=0):    #特征方差
        """
    ...
    var_filter(data, 'y', k=0)

    def vif_test(data, label, k=None):   #共线性检验
        """
    ...
    d4,iv,cut,woe = bin_distince(data['frechet_form8'], data['y'], n =2)
    ...
    d4,iv,cut,woe = bin_frequency(data['frechet_form8'], data['y'], n =3)
